<?php
  ...
  $add = array();
  if(isset($_REQUEST['queijo'])){
  $queijo = $_REQUEST['queijo'];
  $queijopr = 3;
  }

  $ponto = $_REQUEST['ponto'];
  
  if(isset($_REQUEST['alface'])){
  $add['alface'] = $_REQUEST['alface'];
  $add['preco'] = 3;
  } 

  if(isset($_REQUEST['bacon'])){
  $add['bacon'] = $_REQUEST['bacon'];
  $add['preco'] = 3;
  }

  if(isset($_REQUEST['tomate'])){
  $add['tomate'] = $_REQUEST['tomate'];
  $add['preco'] = 3;
  }

  if(isset($_REQUEST['cebola'])){
  $add['cebola'] = $_REQUEST['cebola'];
  $add['preco'] = 3;
  }

  if(isset($_REQUEST['picles'])){
  $add['picles'] = $_REQUEST['picles'];
  $add['preco'] = 3;
  }

  if(isset($_REQUEST['pimenta'])){
  $add['pimenta'] = $_REQUEST['pimenta'];
  $add['preco'] = 3;
  }
}
...
<body>
  ...
  <nav id="navbar"class="navbar navbar-expand-lg navbar-light bg-dark">
    <a class="navbar-brand col-5" href="../ind.php"><img src="img/logo1.png" alt="Logo" id="logo"></a>
    ...
    <div class="collapse navbar-collapse" id="navbarNavDropdown">
      ...
      <?php 
          ...
          ?>
        <li class="nav-item">
          <a class="nav-link text-light" href="../ind.php#quem-somos">Sobre <span class="sr-only">(Página atual)</span></a>
        </li>
        <li class="nav-item">
          <a class="nav-link text-light" href="../ind.php#localizacao">Localização</a>
        </li>
    ...
    <section class="produto">
        <div class="produto-imagem">
            <img src="img/<?php if($produto == "existente"){
              echo $imagem;
            }else{
              echo "hamburgueres.jpeg";
            } ?>" alt="hamburguer">
        </div>
        <div class="produto-informacoes">
            <div class="titulo">
                <p><?php if($produto == "existente"){
                  echo $nome;
                }else{
                  echo $nome;
                  } ?></p>
            ...
            <div class="ingredientes">
              ...
              <?php } ?>
                <p><?php if($produto == "existente"){
                  ...
                  echo "Carne de Hamburguer: " . $burger . "<br>";
            ...
            <div class="preco">
                ...
                <?php if($produto == "existente"){ ?>
                <a href="../controller/controller.php?carrinho_new=1&nome=<?php echo $nome;?>&imagem=<?php echo $imagem;?>&legenda=<?php echo $legenda; ?>&preco=<?php echo $preco;?>&produto=<?php echo $produto;?>" class="rubik">Adicionar ao carrinho</a>
                <?php 
                }else{ 
                  ?>
                <a href="../controller/controller.php?carrinho_new=1&nome=<?php echo $nome;?>&preco=<?php echo $preco;?>&imagem=hamburgueres.jpeg&pao=<?php echo $pao; ?>&burger=<?php echo $burger;?>&produto=<?php echo $produto;?>&ponto=<?php echo $ponto;?>&queijo=<?php if(isset($queijo)){
                  echo $queijo;
                }else{
                  $queijo = "Nenhum";
                  echo $queijo;
                } ;?>&alface=<?php if(isset($add['alface'])){
                  echo $add['alface'];
                }else{
                  $add['alface'] = "Nenhum";
                  echo $add['alface'];
                } ;?>&bacon=<?php if(isset($add['bacon'])){
                  echo $add['bacon'];
                }else{
                  $add['bacon'] = "Nenhum";
                  echo $add['bacon'];
                } ;?>&tomate=<?php if(isset($add['tomate'])){
                  echo $add['tomate'];
                }else{
                  $add['tomate'] = "Nenhum";
                  echo $add['tomate'];
                } ;?>&cebola=<?php if(isset($add['cebola'])){
                  echo $add['cebola'];
                }else{
                  $add['cebola'] = "Nenhum";
                  echo $add['cebola'];
                } ;?>&picles=<?php if(isset($add['picles'])){
                  echo $add['picles'];
                }else{
                  $add['picles'] = "Nenhum";
                  echo $add['picles'];
                } ;?>&pimenta=<?php if(isset($add['pimenta'])){
                  echo $add['pimenta'];
                }else{
                  $add['pimenta'] = "Nenhum";
                  echo $add['pimenta'];
                } ;?>" class="rubik">Adicionar ao carrinho</a>
